$(document).ready(function(){
  ...
  var search_url='/contacts/users'; // if we want to load JSON from file in current directory.
  ...
  $.getJSON(search_url, function(jsonString) { // Get JSON from URL. Save object to LocalStorage. 
    ...
    if(!localStorage.getItem("localObj")) { // if local object do not exist.
      ...
      var serialArr = JSON.stringify(sortArr); // Making object ready for sending to storage. serialization.
      localStorage.setItem('localObj', serialArr); //Saveing serialized object to localStorage.
  ...
  function  generate_cells(){ // function creating cells of our contatc list.
    ...
    for (var key in bookObj) { // running an object elements to generate cells of a contact list.

        $('#print').append(`<div class='book_cell' id='${key}' contenteditable="false"> 
                            <img src="${bookObj[key].avatar}" onError="src='img/userpic.png'"  alt="pic" class="contact_icon noselect"> ${bookObj[key].name} <i class="material-icons chevronUp hide"> expand_less </i></div>`);
        $('#print').append(`<div class='card hide' id='card_${key}'>  
                              ...
                              <table>
                                ...
                                <tr><td></td><td class="edit" id="card_name" contenteditable="false"><h3>${bookObj[key].username} </h3></td></tr>
                                <tr><td> Contacts: </td></tr>
                                <tr><td> <i  class="material-icons"> phone </i> </td><td class="edit" id="card_phone" contenteditable="false">${bookObj[key].phone} </td></tr>
                                <tr><td> <i  class="material-icons"> email </i> </td><td class="edit" id="card_email" contenteditable="false">${bookObj[key].email} </td></tr>
                                <tr><td> <i  class="material-icons"> language </i> </td><td class="edit" id="card_website" contenteditable="false">${bookObj[key].website} </td></tr>
                                <tr><td> Company: </td></tr>
                                <tr><td> <i  class="material-icons"> work </i></td><td class="edit" id="card_company_name" contenteditable="false"><h3>${bookObj[key].company.name}</h3></td></tr>
                                <tr><td></td><td class="edit" id="card_company_bs" contenteditable="false">— ${bookObj[key].company.bs}</td></tr>
                                <tr><td></td><td class="edit" id="card_company_catchPhrase"contenteditable="false"><i>"${bookObj[key].company.catchPhrase}"</i></td></tr>
                              </table>
